...
import { getNextAttemptNumberForEntry } from "../../logic/liftingOrder";
...
class LiftingFooter extends React.Component<Props> {
  ...
  handleLifterChange = (event: React.BaseSyntheticEvent) => {
    ...
    const entry = this.props.orderedEntries.find((entry) => entry.id === entryId);
    if (entry) {
      const lift = this.props.lifting.lift;
      const attempt = getNextAttemptNumberForEntry(entry, liftToAttemptFieldName(lift), liftToStatusFieldName(lift));
      if (attempt > 0) {
        this.props.overrideAttempt(attempt);
      }
    }
